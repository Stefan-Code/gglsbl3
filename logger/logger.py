...
class Logger(object):
    ...
    def __init__(self, name):
        ...
        if not logger.handlers:
            ...
            settings.logger.LOG_DIR
            logging_dir = settings.logger.LOG_DIR
            ...
            handler.setLevel(settings.logger.LOG_LEVEL_FILE)
            logger.addHandler(handler)
            ch = logging.StreamHandler(sys.stdout)
            ch.setLevel(settings.logger.LOG_LEVEL_CONSOLE)
